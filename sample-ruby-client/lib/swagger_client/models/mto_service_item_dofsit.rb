...
module SwaggerClient
  ...
  class MTOServiceItemDOFSIT
    ...
    attr_accessor :pickup_postal_code
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'pickupPostalCode')
        self.pickup_postal_code = attributes[:'pickupPostalCode']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @pickup_postal_code.nil?
        invalid_properties.push('invalid value for "pickup_postal_code", pickup_postal_code cannot be nil.')
      end

      if @pickup_postal_code !~ Regexp.new(/^(\\d{5}([\\-]\\d{4})?)$/)
        invalid_properties.push('invalid value for "pickup_postal_code", must conform to the pattern /^(\\d{5}([\\-]\\d{4})?)$/.')
      end
    ...
    def valid?
      ...
      return false if @pickup_postal_code.nil?
      return false if @pickup_postal_code !~ Regexp.new(/^(\\d{5}([\\-]\\d{4})?)$/)
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] pickup_postal_code Value to be assigned
    def pickup_postal_code=(pickup_postal_code)
      if pickup_postal_code.nil?
        fail ArgumentError, 'pickup_postal_code cannot be nil'
      end

      if pickup_postal_code !~ Regexp.new(/^(\\d{5}([\\-]\\d{4})?)$/)
        fail ArgumentError, 'invalid value for "pickup_postal_code", must conform to the pattern /^(\\d{5}([\\-]\\d{4})?)$/.'
      end
    ...
    def hash
      [id, move_task_order_id, mto_shipment_id, re_service_id, re_service_name, status, rejection_reason, model_type, e_tag, re_service_code, reason, pickup_postal_code].hash
    end
